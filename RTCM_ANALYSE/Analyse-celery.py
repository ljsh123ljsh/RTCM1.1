...
class MyThread(Thread):
    def __init__(self, ar, target=None, args=()):
        self.target = target
...
def analyseWholeFrame(content):
    ...
    for data in content_lis:
        thr = MyThread(ar=data, target=analyse, args=(data,))
...
if __name__ == '__main__':
    ...
    print(pp)
    import json
    print(sys.getsizeof(str(pp).encode()))
    print(sys.getsizeof(json.dumps(pp)))
    print(dict2json_Compress(pp))
